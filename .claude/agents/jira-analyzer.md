...
Score `description_plain` against each of the four fingerprints (inlined at the bottom of this file) **independently**, then pick the highest. Output one of `"BE_Skill"`, `"FE_Skill"`, `"APP_Skill"`, or `null`.

Note: BE plans are emitted by `/prepare-dev-plan` while FE and APP plans are both emitted by `/lfi-prepare-dev-plan` and share the same HTML-comment marker — disambiguation depends on platform-specific markers (Java/Spring vs. web vs. RN/Expo vs. BuilderFax expo-router). Two APP variants exist: LFI Timesheet (RN/Expo) scored in 4c and BuilderFax scored in 4d; the APP score is max(4c, 4d).
...
### 4d. BuilderFax app score (8-item checklist)

Apply the hard-rule checklist from **Appendix D — BuilderFax app fingerprint**:

1. H1 line matches `^# Implementation Plan: .+$`
2. HTML comment `<!-- Source: jira-requirement.txt | Generated by /lfi-prepare-dev-plan -->` present
3. Top-level section order: Goal, Acceptance Criteria, Affected Areas, Architecture Notes, ≥1 `Phase N — <Name>`, Conventions Reminder
4. `## Affected Areas` path column contains entries matching `app/<feature>/`, `components/screens/<Feature>/`, or `scripts/api/<domain>/`
5. `## Architecture Notes` rows include at least one of: "Platform targets", "New route in expo-router", "WebView / iframe embedding compatibility"
6. Per-phase `### State / Store Changes` table has a `Persist allowlist` column
7. Per-phase `### Components & Hooks` table has a `Memoization / Platform notes` column
8. `## Conventions Reminder` Source column cites `CLAUDE.md` or `docs/claude-skills/context/coding_guidelines.md`

Disqualifiers (any one → 0): LFI Timesheet-only markers dominate (`RootStackParamList`, `lumberfi-app-components`, `offlineMode`, `src/services/offlineMode/`) without any BuilderFax path markers (`app/<feature>/`, `components/screens/`, `scripts/api/`).

Map count of passes → 0–5: `8 pass → 5`, `6–7 → 4`, `4–5 → 3`, `2–3 → 2`, `1 → 1`, `0 or any disqualifier → 0`.

### 4e. Decide skill_name

- Compute `app_score = max(step 4c score, step 4d score)`.
- Pick the fingerprint family with the highest 0–5 score: BE (4a), WEB (4b), APP (app_score).
- If all three score `≤ 1`, `skill_name = null`.
- Otherwise: highest BE → `"BE_Skill"`, highest WEB → `"FE_Skill"`, highest APP → `"APP_Skill"`.
- On a tie between the top two, prefer the one with more strong-marker matches (BE §1.1 / WEB items 1–4 / APP items 1–4 from whichever of 4c/4d scored higher). If still tied → `null`.
...
## Appendix D — BuilderFax app fingerprint (`/lfi-prepare-dev-plan`, BuilderFax expo-router app)

Use this checklist to determine whether a given implementation plan was produced by `/lfi-prepare-dev-plan` targeting the BuilderFax expo-router codebase. BuilderFax uses the same skill name and HTML-comment marker as the LFI Timesheet app (Appendix C) but has distinct directory layout and table column conventions.
...
Presence alone is necessary but not sufficient to distinguish BuilderFax from LFI Timesheet — use the structural markers below for disambiguation.

### 2. Top-level sections (mandatory, fixed order)

`Goal`, `Acceptance Criteria`, `Affected Areas`, `Architecture Notes`, one or more `Phase N — <Name>`, `Conventions Reminder`.

### 3. Discriminating structural markers (BuilderFax-specific)

| # | Marker | Where to look |
|---|--------|---------------|
| 1 | `## Acceptance Criteria` table has exactly two columns: `#` and `Criterion` — **no** Status or checkbox column | After `## Goal` |
| 2 | `## Affected Areas` path column contains entries like `` `app/<feature>/` ``, `` `components/screens/<Feature>/` ``, `` `scripts/api/<domain>/` `` | After `## Acceptance Criteria` |
| 3 | `## Architecture Notes` rows include "Platform targets", "New route in expo-router", or "WebView / iframe embedding compatibility needed" | After `## Affected Areas` |
| 4 | Per-phase `### State / Store Changes` table has a `Persist allowlist` column with `yes` / `no` values | Inside each phase |
| 5 | Per-phase `### Components & Hooks` table has a `Memoization / Platform notes` column | Inside each phase |
| 6 | Empty sub-sections rendered as italic `_None_` (never omitted) | Inside any phase sub-section |
| 7 | `## Conventions Reminder` table columns `#`, `Rule`, `Source`; Source values cite `` `CLAUDE.md` `` or `` `docs/claude-skills/context/coding_guidelines.md` `` | Last section |

### 4. Disqualifiers (any one → score 0 for BuilderFax)

- LFI Timesheet-only markers dominate (`RootStackParamList`, `lumberfi-app-components`, `offlineMode`, `src/services/offlineMode/`) **without** any BuilderFax path markers (`app/<feature>/`, `components/screens/`, `scripts/api/`).
- Plan path is `fe-dev-implementation-plan.md` (web SPA, not BuilderFax).
- `## Affected Areas` paths follow web SPA layout (`src/screens/<Feature>/` with no `app/` prefix).
